...
- **承認フローは非原子的な追記。** `recordSettlementApproval`（生 SQL）は変更せず、`approveSettlement` 内で承認確定の直後に `insertPaymentNotice` で通知書を1行 append する。「承認済みだが通知書なし」は §4-1 の通り正規の回復可能状態（再作成で解消）のため、原子性は要件ではないと判断した。`approveSettlement` の戻り値に `noticeCreated` / `noticeSkippedReason` を追加。
