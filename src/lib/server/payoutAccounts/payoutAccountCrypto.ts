...
export const decryptPayload = (
  ...
  if (envelope.v !== ENCRYPTION_KEY_VERSION) {
    throw new Error("Unsupported payout account encryption version");
  }
  const combined = Buffer.from(envelope.data, "base64");
  if (combined.length <= IV_LENGTH + AUTH_TAG_LENGTH) {
    throw new Error("Invalid payout account ciphertext");
  }
  ...
  return JSON.parse(plaintext) as WorkerPayoutAccountPayload;
};
