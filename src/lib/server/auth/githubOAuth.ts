...
export const createGithubAuthorization = (): { state: string; url: string } => {
  ...
  url.searchParams.set("client_id", requireEnv(env.githubClientId, "GITHUB_CLIENT_ID"));
...
export const exchangeGithubCode = async (code: string): Promise<string> => {
  ...
  if (!payload.access_token) {
    throw new Error(payload.error_description ?? payload.error ?? "GitHub token was not returned");
  }

  return payload.access_token;
};

export const fetchGithubUser = async (accessToken: string): Promise<GitHubUserResponse> => {
